...
sub th_size {
	...
	my $devicePixelRatio = $self->{request}->cookies->{'devicePixelRatio'};
	if ($size eq 'large' and my $clientWidth = $self->{request}->cookies->{'clientWidth'} and my $clientHeight = $self->{request}->cookies->{'clientHeight'}) {
		...
		return $self->th_px($px, $width, $height, $devicePixelRatio);
	}
	elsif ($size eq 'small') {
		return $self->th_px(300, $width, $height, $devicePixelRatio);
	}
	else {
		return $self->th_px(800, $width, $height, $devicePixelRatio);
	}
}

sub th_px {
	my $class = shift;
	my ($px, $width, $height, $devicePixelRatio) = @_;
	$devicePixelRatio ||= 1;
	my $max_aspect = $width > $height ? $width : $height;
	if ($max_aspect < $px) {
		return XML::LibXML::NodeList->new(
			XML::LibXML::Text->new($max_aspect * $devicePixelRatio),
		...
		return XML::LibXML::NodeList->new(
			XML::LibXML::Text->new($px * $devicePixelRatio),
